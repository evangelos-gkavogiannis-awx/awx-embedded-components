import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import './WelcomePage.css'; // Add styles for the WelcomePage

const WelcomePage = () => {
    const navigate = useNavigate();
    const [selectedOption, setSelectedOption] = useState(null);

    // Handle option selection
    const handleOptionSelect = (option) => {
        setSelectedOption(option);
    };

    // Handle navigation to KYCForm
    const handleContinue = () => {
        navigate('/kyc-form'); // Redirect to the KYCForm page
    };

    return (
        <div className="welcome-container">
            <header className="welcome-header">
                <img
                    src="data:image/jpeg;base64,"
                    alt="aiwallex Logo"
                    className="airwallex-logo"
                />
                <h1>KYC Embedded demo</h1>
                <p>
                    The financial payments embedded finance financial platform for your global ambition
                </p>
            </header>
            <section className="categories-container">
                <h3>Categories we support:</h3>
                <div className="categories">
                    <div
                        className={`category ${selectedOption === 'Digital products or SaaS' ? 'selected' : ''}`}
                        onClick={() => handleOptionSelect('Digital products or SaaS')}
                    >
                        Digital products or SaaS
                    </div>
                    <div
                        className={`category ${selectedOption === 'Mobile Apps' ? 'selected' : ''}`}
                        onClick={() => handleOptionSelect('Mobile Apps')}
                    >
                        Mobile Apps
                    </div>
                </div>
                <h3>Categories with more restrictions:</h3>
                <div className="categories">
                    <div
                        className={`category ${selectedOption === 'Human services' ? 'selected' : ''}`}
                        onClick={() => handleOptionSelect('Human services')}
                    >
                        Human services
                    </div>
                    <div
                        className={`category ${selectedOption === 'Cloud hosting' ? 'selected' : ''}`}
                        onClick={() => handleOptionSelect('Cloud hosting')}
                    >
                        Cloud hosting
                    </div>
                    <div
                        className={`category ${selectedOption === 'Financial services' ? 'selected' : ''}`}
                        onClick={() => handleOptionSelect('Financial services')}
                    >
                        Financial services
                    </div>
                    <div
                        className={`category ${selectedOption === 'Physical products' ? 'selected' : ''}`}
                        onClick={() => handleOptionSelect('Physical products')}
                    >
                        Physical products
                    </div>
                    <div
                        className={`category ${selectedOption === 'Gambling products' ? 'selected' : ''}`}
                        onClick={() => handleOptionSelect('Gambling products')}
                    >
                        Gambling products
                    </div>
                    <div
                        className={`category ${selectedOption === 'Marketplace for sellers' ? 'selected' : ''}`}
                        onClick={() => handleOptionSelect('Marketplace for sellers')}
                    >
                        Marketplace for sellers
                    </div>
                </div>
            </section>
            <footer className="welcome-footer">
                <div className="terms-container">
                    <input type="checkbox" id="terms" />
                    <label htmlFor="terms">
                        I have read and understood Acceptable use policy, Terms of use & Privacy policy of
                        <a href="https://www.airwallex.com/terms/sign-up-terms" target="_blank" rel="noopener noreferrer">
                            Airwallex
                        </a>.
                    </label>
                </div>
                <button
                    className="continue-button"
                    onClick={handleContinue}
                    disabled={!selectedOption}
                >
                    Continue
                </button>
            </footer>
        </div>
    );
};

export default WelcomePage;
